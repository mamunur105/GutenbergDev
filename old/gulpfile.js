...
gulp.task('blockJS', () => {
	return gulp
		.src(config.blockJSSRC, { since: gulp.lastRun('blockJS') }) // Only run on changed files.
		.pipe(plumber(errorHandler))
		.pipe(
			babel({
				presets: [
		...
		.pipe(remember(config.blockJSSRC))
		.pipe(concat(config.jsBlockFile + '.js'))
		.pipe(lineec())
		.pipe(gulp.dest(config.blockJSDestination))
		.pipe(
			rename({
				basename: config.jsBlockFile,
				suffix: '.min'
			})
		)
		.pipe(uglify())
		.pipe(lineec())
		.pipe(gulp.dest(config.blockJSDestination))
		.pipe(notify({ message: '✅  \n===> Block JS — completed!', onLast: true }));
});
...
gulp.task(
	'run',
	gulp.parallel('styles', 'admin', 'adminJS', 'customJS', 'blockJS', 'images', browsersync, () => {
		...
		gulp.watch(config.watchBlockVendor, gulp.series('blockJS', reload)); // Reload on adminJS file changes.
...
gulp.task(
	'buildDist',
	gulp.series('clean', 'buildStyles', 'buildAdmin', 'adminJS', 'blockJS','customJS', 'images', 'translate', 'buildFiles', 'buildZip', function(done) {
		done();
	})
);
